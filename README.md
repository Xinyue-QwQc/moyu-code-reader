...
[下载安装](https://github.com/Xinyue-QwQc/moyu-code-reader/releases/latest) · [完整使用指南](docs/reading-guide.md) · [上游项目](https://github.com/zwb8926/fanqie-novel-vscode)
...
- 正文与侧边栏支持右键手动刷新。正文刷新保留阅读位置，侧边栏按条目更新阅读进度。
...
![代码伪装模式选择与阅读界面](docs/images/code-camouflage.png)

*代码伪装模式选择器及实际阅读效果，包含正文分色、段间空行与原生彩色缩略图。*
...
![Codex 选区附件与上下文交互](docs/images/codex-selection.png)

*实际阅读界面中的选区右键菜单与 Codex 会话，展示选区附件及用户提交的分析任务。*
...
本扩展通过**番茄小说网页端接口**获取章节内容，阅读权限与网页端保持一致。**完整章节阅读需要番茄 SVIP 权益**；在本扩展中阅读完整小说，需登录具备相应网页端阅读权益的番茄 SVIP 账号。

普通账号可查看网页端开放的免费章节或试读片段，具体可读范围以番茄网页端当前规则及接口返回内容为准。使用前可在番茄网页端确认账号的 SVIP 权益与目标书籍的完整章节访问权限。
...
3. 打开活动栏 **番茄小说**并扫码登录。完整章节阅读使用具备网页端权益的番茄 SVIP 账号，再通过书城、搜索、书架或历史记录选择书籍。
4. 根据阅读需求设置字体、每页章节数和代码伪装模式；使用 AI 分析时，通过正文右键或状态栏交接上下文。

### 版本兼容性

本扩展保留原扩展标识 `zwb8926.fanqie-novel`，用于延续已有书架、设置和阅读进度。安装本仓库的 VSIX 会替换同标识版本，同一扩展标识仅保留一个已安装版本。安装包通过本仓库的 GitHub Release 发布，与上游 Marketplace 发行分别管理。

原生阅读和代码伪装可独立使用。使用 Codex 交互功能时，需另行安装并配置 [Codex IDE 扩展](https://developers.openai.com/codex/ide)。

## AI 辅助开发

本项目由 **[Xinyue-QwQc](https://github.com/Xinyue-QwQc)** 发起维护，使用 **OpenAI Codex** 辅助需求整理、代码实现、问题排查、测试和 README 编写。提交记录中的 `Codex <codex@local.invalid>` 用于标识本地 AI 辅助提交。
